...
class _DeliveryPageState extends State<DeliveryPage> {
  ...
  Future<void> fetchTruckDrivers() async {
    final drivers = await FirebaseFirestore.instance
        .collection('driver')
        .where('vehicleType', isEqualTo: 'Yuk avtomobil')
        .get();
    setState(() {
      allDrivers = drivers.docs; // Store the full list of drivers
      truckDrivers = allDrivers; // Show all drivers initially
    });
  ...
  void filterDrivers() {
    if (selectedFrom != null && selectedTo != null) {
      ...
      setState(() {
        truckDrivers = allDrivers; // Reset to show all drivers if no filters
      });
    }
  }
                  ...
                  child: Theme(
                    ...
                    child: ExpansionTile(
                      leading: ClipRRect(
                        borderRadius: BorderRadius.circular(15),
                        child: Image.asset(
                          'assets/images/truck.png', // Adjusted to a truck image
                          width: 80,
                          height: 80,
                          fit: BoxFit.contain,
                        ),
